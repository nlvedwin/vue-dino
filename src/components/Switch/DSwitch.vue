...
<template>
    <div class="d-flex d-items-center">
        <button
            @click="toggle"
            type="button"
        ...
        >
            <span
                edwin
                class="d-pointer-events-none d-relative d-inline-block d-h-5 d-w-5 d-transform d-rounded-full d-bg-white d-shadow d-ring-0 d-transition d-duration-200 d-ease-in-out"
                :class="{
                    'd-translate-x-5': enabled,
                    'd-translate-x-0': !enabled,
                }"
            >
                <span
                    :class="
        ...
        <span class="d-ml-3 d-text-sm d-capitalize">
            <slot>{{ label }}</slot>
        </span>
    </div>
</template>
